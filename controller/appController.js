...
const categoria = async (req, res) => {
    ...
    //Se envía a la categoria que se seleccione y muestra su respectivo nombre
...
const buscador = async (req, res) => {
    ...
    //Se valida que la variable termino no se encuentre vacía.
    if(!termino.trim()){
    ...
    const propiedades = await Propiedad.findAll({
        where: {
            titulo: { //Columna para habilitar la busqueda.
                [Sequelize.Op.like] : '%' + termino + '%' //Like funciona como like en SQL
            }
        },
        include: [
            { model: Precio, as: 'precio'}
